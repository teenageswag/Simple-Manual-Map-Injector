...
bool Injector::Inject(HANDLE hProc, BYTE* pSrcData, SIZE_T FileSize, bool ClearHeader, bool ClearNonNeededSections, bool AdjustProtections, bool SEHExceptionSupport, DWORD fdwReason, LPVOID lpReserved) {
	...
	if (reinterpret_cast<IMAGE_DOS_HEADER*>(pSrcData)->e_magic != 0x5A4D) { // "MZ"
		log::error("Invalid file format"); // todo: improve log desc
		return false;
	}
	...
	if (pOldFileHeader->Machine != CURRENT_ARCH) {
		log::error("Invalid platform"); // todo: improve log desc
		return false;
	}
	...
	if (!pTargetBase) {
		log::error("Target process memory allocation failed (ex) 0x{}", GetLastError());
		return false;
	}
	...
	if (!WriteProcessMemory(hProc, pTargetBase, pSrcData, 0x1000, nullptr)) { //only first 0x1000 bytes for the header
		log::error("Can't write file header 0x{}", GetLastError());
	...
	for (UINT i = 0; i != pOldFileHeader->NumberOfSections; ++i, ++pSectionHeader) {
		if (pSectionHeader->SizeOfRawData) {
			if (!WriteProcessMemory(hProc, pTargetBase + pSectionHeader->VirtualAddress, pSrcData + pSectionHeader->PointerToRawData, pSectionHeader->SizeOfRawData, nullptr)) {
				log::error("Can't map sections: 0x{}", GetLastError());
				VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
				return false;
			}
		}
	...
	if (!MappingDataAlloc) {
		log::error("Target process mapping allocation failed (ex) 0x{}", GetLastError());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
		return false;
	}

	if (!WriteProcessMemory(hProc, MappingDataAlloc, &data, sizeof(MANUAL_MAPPING_DATA), nullptr)) {
		log::error("Can't write mapping 0x{}", GetLastError());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
	...
	if (!pShellcode) {
		log::error("Memory shellcode allocation failed (ex) 0x{}", GetLastError());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
		VirtualFreeEx(hProc, MappingDataAlloc, 0, MEM_RELEASE);
		return false;
	}
	...
	if (!WriteProcessMemory(hProc, pShellcode, Shellcode, 0x1000, nullptr)) {
		log::error("Can't write shellcode 0x{}", GetLastError());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
		VirtualFreeEx(hProc, MappingDataAlloc, 0, MEM_RELEASE);
	...
	log::info("Mapped DLL at {}", pTargetBase);
	log::info("Mapping info at {}", MappingDataAlloc);
	log::info("Shell code at {}", pShellcode);

	log::info("Data allocated\n");

#ifdef _DEBUG
	log::info("My shellcode pointer %p\n", Shellcode);
	log::info("Target point %p\n", pShellcode);
	system("pause");
#endif
	...
	if (!hThread) {
		log::error("Thread creation failed 0x{}", GetLastError());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
		VirtualFreeEx(hProc, MappingDataAlloc, 0, MEM_RELEASE);
		VirtualFreeEx(hProc, pShellcode, 0, MEM_RELEASE);
		return false;
	...
	log::info("Thread created at: {}, waiting for return...", pShellcode);
	...
	while (!hCheck) {
		...
		if (exitcode != STILL_ACTIVE) {
			log::error("Process crashed, exit code: 0x{}", exitcode);
			return false;
		}

		if (GetTickCount() - tStart > kInjectTimeoutMs) {
			log::error("Injection timed out after {} ms (DllMain may be hung)", kInjectTimeoutMs);
			return false;
		}
		...
		if (hCheck == (HINSTANCE)0x404040) {
			...
			log::warn("WARNING: Exception support failed!");
		}
		...
		for (UINT i = 0; i != pOldFileHeader->NumberOfSections; ++i, ++pSectionHeader) {
			if (pSectionHeader->Misc.VirtualSize) {
				if ((SEHExceptionSupport ? 0 : strcmp((char*)pSectionHeader->Name, ".pdata") == 0) ||
					...
					log::info("Processing {} removal", pSectionHeader->Name);
					if (!WriteProcessMemory(hProc, pTargetBase + pSectionHeader->VirtualAddress, emptyBuffer, pSectionHeader->Misc.VirtualSize, nullptr)) {
						log::error("Can't clear section {}: 0x{}", pSectionHeader->Name, GetLastError());
					}
				}
			}
		}
		...
		for (UINT i = 0; i != pOldFileHeader->NumberOfSections; ++i, ++pSectionHeader) {
			if (pSectionHeader->Misc.VirtualSize) {
				...
				if (VirtualProtectEx(hProc, pTargetBase + pSectionHeader->VirtualAddress, pSectionHeader->Misc.VirtualSize, newP, &old)) {
					log::info("Section {} set as {}", pSectionHeader->Name, newP);
				}
				else {
					log::error("Section {} not set as {}", pSectionHeader->Name, newP);
				}
			}
		}
		DWORD old = 0;
	...
	if (!WriteProcessMemory(hProc, pShellcode, emptyBuffer, 0x1000, nullptr)) {
		log::warn("Can't clear shellcode");
	}
	if (!VirtualFreeEx(hProc, pShellcode, 0, MEM_RELEASE)) {
		log::warn("Can't release shell code memory");
	}
	if (!VirtualFreeEx(hProc, MappingDataAlloc, 0, MEM_RELEASE)) {
		log::warn("Can't release mapping data memory");
	}
